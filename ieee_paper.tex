...
P2: Architecture \& Design &
  ...
  majority vote); API surface design (ubus, UCI schema); initial
  module decomposition; eBPF telemetry architecture; CAKE diffserv4
...
QEMU environment and confirmed on the target hardware
(Section~\ref{subsec:hardware}).
CPU overhead at 1--2~Hz (excluding the startup baseline calibration) stays
below 2\% on a single Cortex-A53 core.
RAM usage includes the 256-entry conntrack LRU, per-device table,
per-flow service table, DNS snoop cache, action ring, and EWMA state.
The IFB, iptables, nftables mangle chain, and BPF map add negligible
kernel-side memory.
...
\begin{tabular}{lll}
\toprule
\textbf{Resource} & \textbf{Budget} & \textbf{Actual (AX3000T)} \\
\midrule
CPU (1--2~Hz loop)    & $<$20\%   & $<$2\% \\
RAM (daemon RSS)     & $<$64~MB  & $\approx$4--5~MB \\
...
Flow stability commit& $<$2~s    & $\approx$1~s (2 ticks at 2~Hz) \\
\bottomrule
\end{tabular}
\end{table}
...
Three controlled experiments demonstrate the system's effectiveness:
a 154$\times$ in-tin delay advantage for gaming traffic over bulk traffic
when per-device classification is correct;
a 38\% reduction in gaming latency under a five-client mixed-persona
workload compared to CAKE-alone;
and correct per-flow service separation when a single source IP
simultaneously runs game, VoIP, bulk, and torrent traffic---a failure
mode that any per-device scheme cannot resolve.
The daemon runs within a $<$2\% CPU budget on a live
Xiaomi~AX3000T deployment and writes zero bytes to flash, preserving
NAND endurance on constrained hardware.
