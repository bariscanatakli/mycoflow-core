...
entirely on-device: it classifies per-device traffic into one of six behavioral
personas without deep-packet inspection, marks flows with DSCP values to steer
them into CAKE diffserv priority tins, and dynamically adjusts the WAN
bandwidth cap through a feedback-driven hysteresis controller.
All learning state is RAM-resident; the daemon writes zero bytes to the router's
NAND flash.
Two QEMU-based controlled experiments demonstrate: (i)~a \textbf{154$\times$
lower average in-tin delay} for gaming traffic vs.\ bulk traffic when per-device
DSCP marking is active, and (ii)~a \textbf{38\% reduction in gaming latency}
under five simultaneous mixed-persona clients compared to CAKE-only AQM.
...
\begin{enumerate}
  ...
  \item A reproducible two-phase QEMU/Docker evaluation covering both
    2-persona and 6-persona workloads, with detailed CAKE tin statistics and
    per-device classification results.
  \item A production-ready OpenWrt package with UCI configuration, procd init
    integration, LuCI dashboard, and zero flash writes during operation.
\end{enumerate}
...
The daemon comprises 12~C11 source modules (Table~\ref{tab:modules})
totalling approximately 2\,800~lines of code, compiled for
...
\texttt{myco\_ubus}    & ubus IPC surface for LuCI dashboard \\
...
\texttt{per\_device}        & 0    & Per-device DSCP marking \\
...
Six CTest suites cover all modules; all 6/6 pass as of the submission date:
\begin{itemize}
  \item \texttt{test\_ewma}: EWMA convergence with $\alpha \in \{0.1, 0.3, 0.9\}$.
  \item \texttt{test\_act}: IFB setup/teardown, interface name injection guard,
    DSCP rule lifecycle (14 test cases).
  \item \texttt{test\_control}: Congestion, clear, outlier, and action
    feedback ring scenarios.
  \item \texttt{test\_config}: UCI parsing, environment variable overrides,
    boundary clamping.
  \item \texttt{test\_persona}: Decision tree coverage, majority-vote window,
    hysteresis transitions.
  \item \texttt{test\_device}: Per-device aggregation, stale eviction,
    DSCP rule update lifecycle.
\end{itemize}
...
P5: Evaluation \& Writing &
  Mar 2026 &
  Real-hardware deployment on Xiaomi~AX3000T (in progress); 6-persona
  mixed-workload benchmark analysis; IEEE conference paper. \\
\bottomrule
\end{tabular}}
\end{table}
...
\emph{This section will present results from a physical deployment on
the target hardware (Xiaomi~AX3000T; MediaTek~MT7981B, 2$\times$Cortex-A53
@ 1.3~GHz, 256~MB LPDDR4, 128~MB NAND, OpenWrt~23.05) and is currently
in progress.}

The real-hardware evaluation plan covers:
\begin{itemize}
  \item CPU and RAM overhead at 1~Hz and 2~Hz under idle and loaded conditions.
  \item Persona classification accuracy against a set of known traffic profiles
    (gaming, VoIP, Netflix streaming, BitTorrent) generated by real devices.
  \item End-to-end gaming RTT and jitter under mixed household load, compared
    against static CAKE and the QEMU baselines from this paper.
  \item Ingress (download) bufferbloat reduction via the IFB path.
\end{itemize}

Preliminary results confirm successful compilation for
\texttt{aarch64-openwrt-linux-musl}, correct UCI configuration loading via the
OpenWrt \texttt{uci} CLI, and nominal daemon startup with procd supervision.
Full quantitative results will be incorporated in the camera-ready version.
