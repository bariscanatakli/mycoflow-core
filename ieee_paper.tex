...
The daemon runs at constant 384\,KB RSS with median 9.5\% CPU on a single
Cortex-A53 core and writes zero bytes to NAND flash.
\end{abstract}
...
mangle chain for per-device DSCP marking, an nftables mangle POSTROUTING
chain that restores per-flow CONNMARKs to DSCP, an optional BPF TC~clsact
...
\begin{enumerate}
  ...
  \item \textbf{Act:} If the persona changed, update CAKE AQM target
    ...
    newly stable flow, push a CONNMARK so the nftables mangle
...
continuous SSH sampling on the live Xiaomi~AX3000T during the Phase~4
in-vivo validation run (9\,387 one-second samples across Modes~A and~B;
Section~\ref{subsec:invivo}).
RSS was allocation-static after start-up: all data structures are
fixed-size circular buffers dimensioned at compile time.

\begin{table}[t]
\caption{Resource Overhead (AX3000T, SSH-sampled, 9\,387 one-second samples)}
\label{tab:resources}
\centering
\scriptsize
{\setlength{\tabcolsep}{3pt}
...
Mem used \%       & 57.4 & 60.0 & 60.7 & 57.0 & 58.9 & 60.5 \\
Conntrack         & 582  & 888  & 1{,}669 & 554 & 785 & 1{,}023 \\
\midrule
mycoflowd RSS   & \multicolumn{3}{c|}{\textbf{384~KB (const.)}} & \multicolumn{3}{c}{\textbf{384~KB (const.)}} \\
Flash (mtd)     & \multicolumn{3}{c|}{0 writes} & \multicolumn{3}{c}{0 writes} \\
Crashes         & \multicolumn{3}{c|}{0 / 4{,}473} & \multicolumn{3}{c}{0 / 4{,}914} \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{enumerate}
\item \textbf{Sub-megabyte RSS.}
  mycoflowd's RSS held at 384\,KB across all 9\,387 samples.
  Flow tables, DNS cache, and per-device state are fixed-size circular
  buffers dimensioned at compile time; no heap growth occurs after
  start-up.
\item \textbf{Single-digit median CPU.}
  Median daemon CPU was 9.5\% on a single 1.3\,GHz A53 core.
  The 46.4\% Mode~A peak correlates with periods of concurrent Windows
  YouTube traffic (system CPU 88.6\%); Mode~B peak was 26.1\%, reflecting
  classifier work under a controlled replay.
  At peak, mycoflowd consumes less than half a core, leaving the second
  core for Wi-Fi acceleration, NAT, and CAKE scheduling.
\item \textbf{Zero flash writes, zero crashes.}
  The router's mtd erase-block counters did not move during the 2.5-hour
  combined run.
  The daemon's \texttt{pidof} probe never returned empty across nearly
  10\,000 samples---a consequence of the deliberate RSS cap and
  allocation-static design that leaves no room for OOM restarts.
\end{enumerate}
...
\begin{itemize}
  \item Median daemon CPU is 9.5\% on a single A53 core at 2~Hz, with a p95
    of 14.7\% during peak classification activity (Mode~B).
  \item RSS is constant at 384~KB across all 9\,387 samples; the daemon is
    allocation-static after start-up.
...
The daemon runs at constant 384\,KB RSS with median 9.5\% CPU on a single
Cortex-A53 core, and writes zero bytes to flash across 9\,387 monitored
seconds.
