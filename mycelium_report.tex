...
Four test suites (CTest) pass with 100\% success rate:
\begin{itemize}
  \item \texttt{test\_ewma}: EWMA convergence with various $\alpha$ values
  \item \texttt{test\_control}: Congestion/clear/outlier scenarios
  \item \texttt{test\_config}: Default values and environment overrides
  \item \texttt{test\_persona}: k-of-m voting and hysteresis transitions
\end{itemize}
