...
Signals: DNS/SNI category, packet-size histogram, inter-arrival variance, flow symmetry, time-of-day prior.
Confidence accumulator:
\[
c_p\leftarrow \beta c_p+(1-\beta)\phi_p(\text{signals}),\quad \beta\in[0,1)
\]
Persona $\arg\max_p c_p$ with sticky hysteresis prevents rapid class flips.
...
\item RAM: $<64$ MB including flow tables; cap maps by LRU evict.
