...
Ubuntu Mode~B & \textbf{33.39\% [31.2,35.7]} & \textbf{48.61\% [46.2,51.0]} & \textbf{66.97\% [64.0,69.8]} \\
\midrule
\multicolumn{4}{l}{Testbed invariance (Mode A, WSL vs Ubuntu): all $|\Delta| < 0.5$\,pt} \\
\multicolumn{4}{l}{DNS lift (Mode B $-$ Mode A, both testbeds): $+12.5$ pt service, $+15.0/+18.0$ pt tin} \\
\multicolumn{4}{l}{McNemar (WSL):  $\chi^2 = 204.00$, $p = 5.0\!\times\!10^{-45}$, $b{=}206$, $c{=}0$, $N{=}1650$}\\
\multicolumn{4}{l}{McNemar (Ubuntu): $\chi^2 = 215.00$, $p = 2.1\!\times\!10^{-47}$, $b{=}217$, $c{=}0$, $N{=}1650$}\\
\bottomrule
\end{tabular}
\end{table}

The deployment-realistic metric (tin-matched, Ubuntu Mode~B) is
\textbf{66.97\%}: when the daemon's classifier observed a flow at all,
two flows out of three were placed in the correct CAKE tin. The
strictest metric (service-level) is the harshest because Unicauca's
nDPI ground truth distinguishes labels — for example
\texttt{video\_live} vs \texttt{video\_vod} — that map to the
\emph{same} tin and therefore receive the same QoS treatment from the
router; a service-level miss between sibling video labels does not
affect the user-visible outcome. Per-class precision/recall is reported
in Table~\ref{tab:invivo_perclass}.
...
video\_vod        & 0.000 & 0.000 & 0.734 & \textbf{0.773} & \textbf{+0.773} \\
...
vivo on the live router, \textbf{66.97\% CAKE tin accuracy} on flows
the classifier successfully observes (Ubuntu testbed, Mode~B; 95\% CI
64.0\%--69.8\%). Service-level accuracy on the same run is 33.4\%, the
gap reflecting Unicauca label distinctions (e.g.\ video\_live vs
video\_vod) that map to the same QoS tin and so do not affect
user-visible behavior. The DNS hint is the dominant signal: it
contributes a $+12.5$\,pt service-level lift and $+18$\,pt tin-level
lift, statistically significant at $p < 2\!\times\!10^{-47}$ via
paired McNemar test, with zero flows where DNS \emph{degraded} the
verdict. Mode~A is testbed-invariant ($|\Delta| < 0.5$\,pt across WSL2
