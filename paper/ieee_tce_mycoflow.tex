...
\paragraph{Results}
We ran the full N=1{,}650 stratified-random replay on \emph{two} test
hosts: a WSL2 client (Windows 11 host, NAT'd Ethernet emulation, with
known TCP egress timeouts to certain CIDR blocks) and a bare-metal
Ubuntu 24.04 server (1\,Gbps direct LAN, all outbound TCP unrestricted).
Mode A is platform-invariant ($\Delta < 0.5$\,pt across all metrics),
which serves as our control: any DNS lift observed in Mode B is
attributable to the hint signal itself rather than testbed network
artifacts. Table~\ref{tab:invivo_overall} summarizes both runs.
...
brackets. Three accuracy metrics are reported: \emph{service-level} (the
strict 11-class label match against the Unicauca ground truth);
\emph{tin-level} (the 4-class CAKE diffserv4 tin assignment, which is what
the router actually acts on); and \emph{tin-matched} (tin-level accuracy
restricted to flows the daemon's classifier table successfully observed,
i.e. the deployment-realistic operating point). McNemar pairing test
compares Mode~A vs Mode~B on the same input flows.}
\label{tab:invivo_overall}
\begin{tabular}{l|ccc}
\toprule
...
Ubuntu Mode~B (585-cat.) & \textbf{37.88\% [35.6,40.3]} & \textbf{52.42\% [50.0,54.8]} & \textbf{70.41\% [67.6,73.1]} \\
\midrule
\multicolumn{4}{l}{Testbed invariance (Mode A, WSL vs Ubuntu): all $|\Delta| < 0.5$\,pt} \\
\multicolumn{4}{l}{Catalog impact (Mode B, 80\,$\rightarrow$\,585 entries): $+4.5$\,pt service, $+3.4$\,pt tin-matched} \\
\multicolumn{4}{l}{DNS lift (Mode B 585-cat. $-$ Mode A): $+17.6$\,pt service, $+21.2$\,pt tin-matched} \\
\multicolumn{4}{l}{McNemar (WSL):  $\chi^2 = 204.00$, $p = 5.0\!\times\!10^{-45}$, $b{=}206$, $c{=}0$, $N{=}1650$}\\
\multicolumn{4}{l}{McNemar (Ubuntu, 80-cat.): $\chi^2 = 215.00$, $p = 2.1\!\times\!10^{-47}$, $b{=}217$, $c{=}0$}\\
\multicolumn{4}{l}{McNemar (Ubuntu, 585-cat.): $\chi^2 = 289.00$, $p = 1.8\!\times\!10^{-63}$, $b{=}291$, $c{=}0$}\\
\bottomrule
\end{tabular}
\end{table}

The deployment-realistic metric (tin-matched, Ubuntu Mode~B with the
expanded 585-domain catalog) is \textbf{70.41\%} [67.6\%, 73.1\%]: when
the daemon's classifier observed a flow at all, more than seven flows
out of ten were placed in the correct CAKE tin. The catalog expansion
from 80 to 585 entries (a $+42$\,KB read-only addition to the binary,
no algorithmic change) lifted service-level accuracy from 33.4\% to
37.9\%, with the largest per-class wins on \texttt{file\_sync}
(20.7\%\,$\rightarrow$\,43.3\%) and \texttt{game\_rt}
(50.7\%\,$\rightarrow$\,76.0\%). The strictest metric (service-level)
remains the harshest because Unicauca's nDPI ground truth distinguishes
labels — for example \texttt{video\_live} vs \texttt{video\_vod} — that
map to the \emph{same} tin and therefore receive the same QoS treatment
from the router; a service-level miss between sibling video labels does
not affect the user-visible outcome. Per-class precision/recall is
reported in Table~\ref{tab:invivo_perclass}.

\begin{table}[t]
\centering
\caption{Per-class precision and recall under Mode~A and Mode~B
(Ubuntu testbed). $\Delta$ Rec. shows the per-class lift from DNS.}
\label{tab:invivo_perclass}
...
The DNS contribution is statistically dominant: McNemar's test rejects
the null of equal classifiers at $p < 10^{-63}$ (Ubuntu 585-cat. run),
with 291 flows correctly classified by Mode~B but missed by Mode~A and
\emph{zero} flows where DNS degraded the verdict ($b=291$, $c=0$). The
per-class lifts cluster into three patterns:

\begin{itemize}
\item \textbf{DNS-decisive (large positive $\Delta$):} \texttt{video\_vod}
goes from 0\% recall to 86.0\%, the strongest lift in the dataset.
\texttt{game\_rt} jumps from 22.7\% to 76.0\% and \texttt{file\_sync}
from 0\% to 43.3\%. These flows are HTTPS to YouTube/Netflix/Steam/
Dropbox CDNs over TCP/443 or UDP/443 (QUIC) — generic ports that no
port hint can disambiguate. The expanded 585-domain DNS catalog
populates $h_{dns}$ from observed \texttt{*.googlevideo.com},
\texttt{*.nflxvideo.net}, \texttt{*.dropboxusercontent.com},
\texttt{*.steampowered.com} resolutions, after which the verdict is
unambiguous.

\item \textbf{DNS-supplementary (moderate positive $\Delta$):}
\texttt{video\_conf} (+0.100, hitting 99.3\% recall) — the few Zoom/
Meet/Teams flows that don't match the STUN port range (3478, 19302–
19309) now resolve via the full domain catalog. Classical port hints
continue to dominate for the remaining UDP STUN game-server flows.
...
overfitting, $\Delta < 0.5$\,pts vs the calibration window) and, in
vivo on the live router with the full 585-domain DNS catalog,
\textbf{70.41\% CAKE tin accuracy} on flows the classifier successfully
observes (Ubuntu testbed, Mode~B; 95\% CI 67.6\%--73.1\%). Service-level
accuracy on the same run is 37.9\%, the gap reflecting Unicauca label
distinctions (e.g.\ \texttt{video\_live} vs \texttt{video\_vod}) that
map to the same QoS tin and so do not affect user-visible behavior.
The DNS hint is the dominant signal: it contributes a $+17.6$\,pt
service-level lift and $+21.2$\,pt tin-matched lift, statistically
significant at $p < 2\!\times\!10^{-63}$ via paired McNemar test, with
zero flows where DNS \emph{degraded} the verdict. Mode~A is testbed-invariant ($|\Delta| < 0.5$\,pt across WSL2
and bare-metal Ubuntu), confirming the lift is attributable to the
classifier and not the test environment. Resource overhead remains
...
14{,}301 monitored seconds across the two runs. We release the
source, test scripts, and per-flow validation dataset under the
project repository to enable replication.
